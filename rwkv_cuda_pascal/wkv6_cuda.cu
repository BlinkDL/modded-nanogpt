    ...
    F *__restrict__ const _gr, F *__restrict__ const _gu)
{
    ...
    for (int t = t_0; t < t_T; t += C)
    {
        ...
        const float k = _k[t];
        const float w = __expf(-__expf(_w[t]));
        float gr = 0, gu_ = 0;
    ...
    F *__restrict__ const _gw)
{
    ...
    for (int t = t_T_1; t > t_1; t -= C)
    {
        ...
        const float r = _r[t];
        const float w = __expf(-__expf(_w[t-C]));
        float sum = 0.0f;
        ...
        sbbbb[(t-t_2)/C] = sum * _k[t-2*C];
    }

    float sss = sbbbb[0];
    _gw[t_0] = 0;
    _gw[t_1] = F(sss * -__expf(_w[t_1]));
    ...
    for (int t = t_2; t < t_T_1; t += C)
    {
        ...
        sss += sbbbb[(t-t_1)/C] - (sum * _r[t]);
        _gw[t] = F(sss * -__expf(_w[t]));
    }
    _gw[t_T_1] = 0;
}
